...
- No per-person privacy walls in v1 — flat family visibility: a daughter's private note is visible to the whole family (set this expectation at onboarding). Designed so per-person visibility is a couple hours' work if it ever chafes.
...
- **FR2 — Message ingestion via email.** School emails auto-forwarded to a dedicated app mailbox are polled, parsed into entries, and marked read. Covers IServ mail redirection and Schulmanager Elternbrief emails; generalizes to any future email-sending school app.
...
- **FR4 — Unified entry model.** Appointments, ideas, tasks, ingested messages, and AI suggestions are one `entry` concept distinguished by type/status/source; an idea-note ("idea" for short) attaches to any entry via a parent relationship.
...
- **FR6 — Reminders via push.** An entry with a remind-at time fires a scheduled push notification at the set time (± 1 min); scheduling survives app restarts.

### Communication (G3)

- **FR7 — Chat with media.** Real-time family chat with persisted history; images and voice notes upload over HTTP. [ASSUMPTION] History is retained indefinitely (family-scale volume); uploads capped at ~25 MB/file; media types are images + voice notes.
- **FR8 — On-demand live location.** A user starts a time-boxed share; others see the live position on a map; the share auto-expires; nothing is persisted. Sharing is initiated by the person being located ("share when asked, and it stops automatically, so I keep control") — teen agency, not parent-side tracking.

### Intelligence (G4)

- **FR9 — AI idea-suggestions.** The app proposes concrete ideas for an entry on demand (user taps "suggest ideas"), and [ASSUMPTION] proactively at most once per upcoming family-created event with an occasion character (birthday, party) — a volume guard against the notification-fatigue counter-metric. Ideas (presents, cake, themes) are in German. Suggestions arrive as proposals the user accepts or dismisses; only the minimum context (event title + type) leaves the system. Provider is swappable (see addendum).

### Access

- **FR10 — Passwordless auth.** Magic-link login via email (sent through the German sovereign mailbox); long-lived sessions. Invite-by-hand for 5 people.
...
Per-person visibility (`family` | `owner_only`) · native app shell if reliable iOS push or background GPS becomes a need · local-LLM adapter (sovereignty upgrade) · minimal admin UI. None of these are gaps — each is a conscious "later, if needed," and the architecture is shaped so adding them is cheap.
