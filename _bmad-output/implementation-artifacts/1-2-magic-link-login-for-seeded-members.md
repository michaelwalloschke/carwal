...
Adversarial review 2026-07-07 (Blind Hunter + Edge Case Hunter + Acceptance Auditor, parallel): 3 decision-needed, 13 patch, 1 defer, 1 dismissed (remember-me-dropped-on-confirm claim refuted — LiveView JS injects the submitter name/value as a hidden input before the native `phx-trigger-action` submit).

- [x] [Review][Patch] (medium) *(resolved decision: remove gate)* Drop `require_sudo_mode` from settings — it made the only authenticated page (and `signed_in_path/1` target) bounce to a full magic-link re-login >10 min after login, contradicting FR10's never-re-prompt; also removes the `true = Accounts.sudo_mode?(user)` stale-submit crash [lib/carwal_web/live/user_live/settings.ex:4,76]
- [x] [Review][Patch] (medium) *(resolved decision: throttle now)* Minimal magic-link throttle — skip minting/sending while an unexpired login token (15-min window) exists for the user; response stays byte-identical, so no enumeration surface is added [lib/carwal_web/live/user_live/login.ex:68]
- [x] [Review][Defer] Login timing oracle — seeded email triggers token insert + synchronous SMTP before the flash renders, unseeded returns immediately; bytes identical (AC2 satisfied), timing discloses seeded addresses [lib/carwal_web/live/user_live/login.ex:72] — deferred per operator decision: household threat model, attacker only learns "email belongs to a family member"; AC2 requires byte-identity, not timing-identity.
- [x] [Review][Patch] (high) Malformed magic-link token → MatchError → 500 on anonymous POST `/users/log-in` — `login_user_by_magic_link/1` hard-matches `{:ok, query}` but `verify_magic_link_token_query/1` returns bare `:error` for non-base64url input. The prior review's defer rationale ("user-facing path uses the safe `with` variant") is wrong: `UserSessionController.create/2` calls the raising variant directly [lib/carwal/accounts.ex:174]
- [x] [Review][Patch] (high) POST `/users/log-in` without `user[token]` → FunctionClauseError → 500 — the login form native-posts `user[email]` when JS is disabled or the user submits before the LiveView socket connects; the email-handling controller clause was removed with the password strip and no fallback was added [lib/carwal_web/controllers/user_session_controller.ex:16]
- [x] [Review][Patch] (medium) Session-token reissue never deletes the superseded token — with validity raised 14→3650 days, every reissued (≥7-day-old) token remains a live credential for ~10 years (the generator's 14-day validity was the implicit GC); logout deletes only the current token; expired "login" tokens are never purged either [lib/carwal_web/user_auth.ex:93-118]
- [x] [Review][Patch] (medium) Mail delivery failures silently swallowed — the `{:error, _}` results of `deliver_login_instructions/2` and `deliver_user_update_email_instructions/3` are discarded; an SMTP outage means the member is told the link/confirmation is on its way while nothing is sent or logged [lib/carwal_web/live/user_live/login.ex:72; lib/carwal_web/live/user_live/settings.ex:80]
- [x] [Review][Patch] (medium) NFR3: custom changeset errors render in English — "must have the @ sign and no spaces" and "did not change" have no entries in `de/LC_MESSAGES/errors.po` (stock Ecto messages do); the settings tests pin the English strings [lib/carwal/accounts/user.ex:35,51]
- [x] [Review][Patch] (low) Double-use/concurrent magic-link POST → `Repo.delete!(token)` raises Ecto.StaleEntryError → 500 for the second request (two tabs; `phx-disable-with` only guards same-button double-clicks) [lib/carwal/accounts.ex:183]
- [x] [Review][Patch] (low) Empty-string env email (`FAMILY_*_EMAIL=""`) bypasses the seed placeholder guard, then `{:ok, _} = register_user(%{email: ""})` MatchErrors mid-list with earlier members already inserted (no transaction) [priv/repo/seeds.exs:26,43]
- [x] [Review][Patch] (low) `SMTP_USERNAME=""` passes the prod mailer guard (empty string is truthy) → boots with empty credentials instead of the intended controlled failure; non-numeric `SMTP_PORT` → bare ArgumentError that doesn't name the variable [config/runtime.exs:76,84]
- [x] [Review][Patch] (low) Test gaps: no remember-me-cookie assertion on the magic-link confirm POST path; seed test doesn't assert the users carry the configured emails; forged-token test doesn't assert no session was established [test/carwal/seeds_test.exs; test/carwal_web/controllers/user_session_controller_test.exs]
- [x] [Review][Patch] (low) Dead code from the password strip: login mount reads a `:email` flash that nothing writes; settings assigns `:current_email` that no template renders [lib/carwal_web/live/user_live/login.ex:59; lib/carwal_web/live/user_live/settings.ex:54]
- [x] [Review][Patch] (low) Planning docs still specify ~1-year sessions while the code implements ~10 years — the same FR10 lines were edited in this commit for the headcount fix [_bmad-output/planning-artifacts/epics.md:28,135; _bmad-output/planning-artifacts/prds/prd-carwal-2026-07-05/prd.md:61]
- [x] [Review][Patch] (low) AGENTS.md (generator-injected block) documents `redirect_if_user_is_authenticated` and registration-page guidance that doesn't exist in this passwordless, no-sign-up codebase — a future agent following it re-introduces removed surface; add a CarWal note [AGENTS.md:57]
- [x] [Review][Patch] (low) Story File List claims `config/test.exs` (−bcrypt log_rounds) and `layouts/root.html.heex` (−dup menu) were modified; both are byte-identical to baseline in the final diff [_bmad-output/implementation-artifacts/1-2-magic-link-login-for-seeded-members.md:173]
- [x] [Review][Defer] No UI path to the login page for a logged-out user — `/` is the starter promo without `Layouts.app`, every other route requires auth; login is reachable only by typing the URL [lib/carwal_web/router.ex:23] — deferred, already documented in Completion Notes; Epic 2 replaces `/` with the authenticated agenda LiveView.
...
**Modified:** `mix.exs` (−bcrypt, +gen_smtp), `mix.lock`, `config/config.exs` (:family_members placeholders), `config/runtime.exs` (SMTP + :family_members via env), `lib/carwal/accounts.ex` (password strip + imports), `lib/carwal/accounts/{user,user_token,user_notifier}.ex`, `lib/carwal_web/user_auth.ex` (cookie 3650), `lib/carwal_web/controllers/user_session_controller.ex`, `lib/carwal_web/live/user_live/{login,settings,confirmation}.ex`, `lib/carwal_web/router.ex` (−register, −update-password), `lib/carwal_web/components/layouts.ex` (nav shell), `priv/repo/seeds.exs`, `test/support/{conn_case,data_case}.ex`, `AGENTS.md` (generator injection). (`config/test.exs` and `layouts/root.html.heex` saw intra-story churn that netted to zero vs the baseline — they are byte-identical in the final diff and deliberately not listed.)
...
- 2026-07-07: Adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor, parallel). 3 decisions resolved by operator (drop settings sudo gate; throttle magic-link requests now; accept login timing oracle), 15 patches applied, 2 deferred, 1 finding dismissed as refuted. Highlights: fixed two reachable 500s on the anonymous login POST (malformed token MatchError — the 2026-07-06 review's defer rationale "user-facing path uses the safe `with` variant" was **wrong**, the controller called the raising variant; and the no-JS/pre-socket-connect email POST that lost its controller clause in the password strip — now served by a shared `Accounts.request_login_link/2` with seeded-gate + one-live-link throttle); superseded session tokens are now revoked on reissue (10-year validity removed the generator's implicit 14-day GC); mail delivery failures are logged (and a failed send no longer consumes the throttle window); custom changeset errors translated (NFR3); seeds/SMTP env hardening; dead password-strip code removed; planning docs aligned to ~10-year sessions; AGENTS.md CarWal note; File List corrected. `mix precommit` green (100 passed). Status → done.